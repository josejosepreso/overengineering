...
selectVideo videoList = do
  ...
  hFlush stdout
  i <- getLine
  callCommand $ "mpv https://youtu.be/" ++ (videoId $ list !! (read i :: Int))

oneSpace :: [String] -> [String]
oneSpace = map (intercalate " ") . map words

search query = curlGetString (init query) [CurlNoProgress True]
               >>= selectVideo . getResults . oneSpace . lines . snd
